...
(definst flute
  [note 60 dur 1.0 level 1 rate 1 loop? 0
   attack 0 decay 1 sustain 1 release 0.1 curve -4 gate 4]
  (let [buf (index:kr (:id index-buffer) note)
        env (env-gen (adsr 0 0 1 0 level curve)
                     ;;(adsr attack decay sustain release level curve)
                     :gate gate
                     :action FREE)]
